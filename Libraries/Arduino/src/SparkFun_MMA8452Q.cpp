...
byte MMA8452Q::readRegister(MMA8452Q_Register reg)
{
	...
	int timeout = 0;
	while(!Wire.available()) { //Wait for the data to come back. If not, timeouts
...
void MMA8452Q::readRegisters(MMA8452Q_Register reg, byte *buffer, byte len)
{
	...
	int timeout = 0;
	while(Wire.available() < len) { //Hang out until we get the # of bytes we expect. If not, timeouts
		delay(RESPONSE_INTERVAL);
		timeout += RESPONSE_INTERVAL;
		if(timeout > RESPONSE_TIMEOUT) {
			break;
